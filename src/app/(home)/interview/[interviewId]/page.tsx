...
import { AlertCircle, Calendar, Clock, Star, ArrowLeft, Play, HelpCircle, Briefcase, Code, Target, AlertTriangle, Sparkles, Edit } from "lucide-react";
import { useAuth } from "@/hooks/useAuth";
import { useErrorRecovery } from "@/hooks/useErrorRecovery";
import { useNetworkStatus } from "@/hooks/useNetworkStatus";
import { getAInterviewDetail, completeInterview, updateUserInterviewStats, saveInterviewProgress, updateInterview } from "@/actions/FirebaseUserApi";
...
// Feedback data type that matches the InterviewFeedback component expectations
type FeedbackItem = {
  id: string;
  question: string;
  userAnswer: string;
...
const convertFirebaseToInterviewData = (firebaseInterview: InterviewType): InterviewData | null => {
  ...
  // Return null if no questions exist - no demo fallback
...
// Enhanced loading component - using new EnhancedLoading component
...
const InterviewPreviewCard = ({ 
  ...
  firebaseData?: InterviewType;
  onStartInterview: () => void;
}) => {  // Helper function to check if interview date is today
  const isInterviewToday = () => {
    if (!firebaseData?.date) return false;
    
    try {
      let interviewDate: Date;
      
      // Handle different date formats from Firebase
      if (typeof firebaseData.date === 'object' && 'seconds' in firebaseData.date) {
        // Firebase Timestamp object
        interviewDate = new Date((firebaseData.date as { seconds: number }).seconds * 1000);
      } else if (typeof firebaseData.date === 'number') {
        // Timestamp number
        interviewDate = new Date(firebaseData.date);
      } else {
        // String date
        interviewDate = new Date(firebaseData.date);
      }
      ...
      console.error('Error checking interview date:', error);
      return false;
    }
  };

  // Check if interview is completed
  const isCompleted = firebaseData?.status === 'completed' || firebaseData?.completedAt;
  
  // Check if interview can be started
  const canStartInterview = isInterviewToday() && !isCompleted;
  
  // Determine the status badge and message
  const getStatusInfo = () => {
    if (isCompleted) {
      return {
        badge: { variant: "default" as const, text: "Completed", className: "bg-green-500 text-white" },
    ...
    if (!isInterviewToday()) {
      return {
        badge: { variant: "destructive" as const, text: "Not Today", className: "" },
        message: "Interview can only be started on the scheduled date"
      };
    }
    
    return {
      badge: { variant: "secondary" as const, text: "Ready to Start", className: "" },
      message: "Get ready for your interview session"
    };
  };
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Badge 
                ...
                className={`text-sm ${statusInfo.badge.className}`}
              >
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="flex items-center space-x-3 p-3 bg-background/60 rounded-lg border">
                ...
                <div>
                  <p className="text-sm font-medium">Scheduled</p>                  <p className="text-xs text-muted-foreground">
                    {firebaseData?.date 
                      ? new Date(typeof firebaseData.date === 'object' && 'seconds' in firebaseData.date 
                          ? (firebaseData.date as { seconds: number }).seconds * 1000 
                          : firebaseData.date
                        ).toLocaleDateString()
                      : 'Not scheduled'
                    }
            ...
            {!canStartInterview && (
              <Alert className="mb-6">
                ...
                <AlertDescription>
                  {isCompleted 
                    ? "This interview has been completed. You can view the results in your dashboard."
                    : "This interview can only be started on the scheduled date. Please check back on the interview date."
                  }
                </AlertDescription>
              </Alert>
            )}
            ...
            {isCompleted ? (
              <Button 
                onClick={() => window.open(`/dashboard/interview/${firebaseData?.id}`, '_blank')}
              ...
              >
                ...
                {canStartInterview ? 'Start Interview' : 'Cannot Start Interview'}
              </Button>
            )}
          </CardContent>
        </Card>
...
// No Questions Card Component - for interviews without questions
const NoQuestionsCard = ({ 
  ...
  const handleGenerateQuestions = async () => {
    ...
    try {      // Generate questions based on interview metadata
      const questions = await generateInterviewQuestions({
        field: (firebaseData.field as "it" | "cs" | "lg" | "other") || 'it',
        type: (firebaseData.type as "frontend" | "backend" | "fullstack" | "devops" | "other") || 'fullstack',
        difficulty: (firebaseData.difficulty as 'beginner' | 'intermediate' | 'advanced') || 'intermediate',
        customType: firebaseData.customType || undefined, // Pass custom data for question generation
        count: 50
      });

      // Convert to interview format
      const formattedQuestions = convertQuestionsToInterviewFormat(questions);      // Update the interview in Firebase
      const updatedInterview: Partial<InterviewType> = {
        questions: formattedQuestions,
        updatedAt: Date.now()
      };

      const response = await updateInterview(firebaseData.id, userData.uid, updatedInterview);
      if (response.success) {
        onQuestionsGenerated();
      } else {
      ...
      <Card className="border-orange-200 bg-orange-50/50">
        <CardHeader>
          <div className="flex items-center space-x-2">
            ...
            <CardTitle className="text-orange-800">No Questions Available</CardTitle>
          </div>          <CardDescription className="text-orange-700">
            This interview doesn&apos;t have any questions yet. You can generate questions tailored to the interview requirements.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
            ...
            <div className="flex items-center space-x-3 p-3 bg-background/60 rounded-lg border">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {firebaseData.field === 'it' ? 'Information Technology' :
                   firebaseData.field === 'cs' ? 'Computer Science' :
                   firebaseData.field === 'lg' ? 'Languages' : 
                   firebaseData.customType || 'General'}
                </p>
              </div>
            </div>
          ...
          <div className="border rounded-lg p-6 bg-gradient-to-r from-blue-50 to-purple-50">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Generate 50 tailored interview questions using AI based on your interview requirements
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">              <div className="bg-white rounded-lg p-4 border">
                ...
                <ul className="text-sm text-muted-foreground space-y-1">
                  <li>• 50 questions tailored to {firebaseData.type || 'full stack'} development</li>
                  <li>• {firebaseData.difficulty || 'Intermediate'} difficulty level</li>
                  <li>• Questions covering theoretical and practical aspects</li>
                  <li>• Instant generation using advanced AI</li>
                </ul>
              </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Button 
              variant="outline" 
              onClick={() => window.open(`/interview/${firebaseData.id}/edit`, '_blank')}
              className="flex-1"
            >
...
function InterviewContent({ firebaseData }: { firebaseData?: InterviewType }) {
  ...
  const [modalType] = useState<'warning' | 'violation' | null>(null);    const [isGeneratingFeedback, setIsGeneratingFeedback] = useState(false);
  ...
  const handleEmailResults = async () => {
    try {
      ...
      await emailResults(emailData, 'user@example.com'); // Add email parameter
    } catch (error) {
  ...
  const handleQuestionSelect = (index: number) => {
    // Only allow navigation to answered questions or the current question + 1
    if (index <= currentQuestionIndex + 1 && 
        (index === currentQuestionIndex || 
         index === currentQuestionIndex + 1 || 
         (interview && completedQuestionIds.includes(interview.questions[index].id)))) {
      // Direct navigation to the question
      if (index < currentQuestionIndex) {
        // Navigate backwards multiple steps if needed
        for (let i = currentQuestionIndex; i > index; i--) {
          previousQuestion();
        }
      } else if (index > currentQuestionIndex) {
        // Navigate forward (should only be possible to the next question)
        nextQuestion();
      }
    }
  };  // Handle next question
  const handleNextQuestion = async () => {    if (interview && currentQuestionIndex === interview.questions.length - 1) {
      // Save current answer if it exists before completing
      if (currentAnswer.trim() && currentQuestion) {
        saveAnswer(currentQuestion.id, currentAnswer);
      }
      
      // Interview completed - generate AI feedback and save results
      try {
        ...
        const timeSpent = interview.duration || 45; // Default fallback
        
        setAiProgress(30);
        setAiStage('Analyzing your responses...');        // Filter out unanswered questions for AI analysis
        const answeredQuestions = interview.questions.filter((q) => 
          answers.some((a: Answer) => a.questionId === q.id && a.text.trim() !== '')
        );
        
        // Use error recovery for AI feedback generation
        const aiFeedback = await errorRecovery.executeWithRetry(async () => {
          return await generateAIFeedback(
            ...
            answers.filter((a: Answer) => a.text.trim() !== ''), // Only include non-empty answers
            interview.id, 
            firebaseData?.userId || '', 
        ...
        const answeredCount = answers.filter((a: Answer) => a.text.trim() !== '').length;
        const completionRate = (answeredCount / totalQuestions) * 100;
          // Prepare comprehensive interview results
        const interviewResults: InterviewResults = {
          summary: {
            overallScore: aiFeedback.length > 0 ? Math.round(aiFeedback.reduce((sum: number, item: FeedbackItem) => sum + (item.score || 0), 0) / aiFeedback.length) : 0,
            totalQuestions: totalQuestions,
            answeredQuestions: answeredCount,
            timeSpent: timeSpent,
            averageTimePerQuestion: timeSpent / totalQuestions,
            performance: (() => {
              if (completionRate < 50) return 'poor' as const;
              const avgScore = aiFeedback.length > 0 ? aiFeedback.reduce((sum: number, item: FeedbackItem) => sum + (item.score || 0), 0) / aiFeedback.length : 0;
              return avgScore >= 80 ? 'excellent' as const : avgScore >= 70 ? 'good' as const : avgScore >= 60 ? 'average' as const : 'poor' as const;
            })()
          },
        ...
        // Save interview completion and results to Firebase
        if (firebaseData?.userId) {
          await errorRecovery.executeWithRetry(async () => {
            await Promise.all([
              // Complete the interview in Firebase
        ...
        console.error('Error generating AI feedback:', error);        // Fallback to basic feedback on error
          ...
          <div className="lg:col-span-6">
            {currentQuestion && (
              <div className="space-y-6">
                ...
                <Card>
                  <CardContent className="pt-6">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">                      <InterviewControls 
                        ...
                        onSave={handleSave}
                        onExit={handleExit}
                        canGoNext={currentQuestionIndex === interview.questions.length - 1 ? true : !!currentAnswer.trim()}
...
export default function InterviewPage() {
  ...
  }, [firebaseData]);  // Load interview data when component mounts or dependencies change
  useEffect(() => {
  ...
  }, [userData?.uid, interviewId]);  // Show loading state
  if (isLoading) {
  ...
  // Show no questions state (when interview exists but has no questions)
  if (firebaseData && !interviewData) {
    const handleQuestionsGenerated = () => {
      // Reload the interview data to get the new questions
      if (userData?.uid && interviewId) {
        setIsLoading(true);
        getAInterviewDetail(userData.uid, interviewId)
          .then(response => {
          ...
          .catch(err => {
            console.error("Error reloading interview:", err);
            setError("Failed to reload interview data");
          })
          .finally(() => {
            setIsLoading(false);
          });
  ...
  // If we don't have firebase data or interview data at this point, it's an error
  if (!firebaseData || !interviewData) {
    return (
      <InterviewErrorCard 
  ...
  const InterviewWrapper = () => {
    ...
    }, [setInterview, contextInterview]); // Removed interviewData from dependencies
